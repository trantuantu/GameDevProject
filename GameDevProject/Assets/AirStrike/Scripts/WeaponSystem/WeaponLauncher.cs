...
public class WeaponLauncher : WeaponBase
{
	...
	private void Start ()
	{
        ...
        if (GameObject.FindGameObjectWithTag("Spawner") != null)
            if (GameObject.FindGameObjectWithTag("Spawner").GetComponent<EnemySpawner>().isBoss == true && this.transform.parent.tag.Equals("Enemy"))
        {
            TargetTag[0] = "Player";
            
        }
        else TargetTag[0] = "Enemy";
        ...
        if (File.Exists("weaponStat"))
        {
            ...
            if (weaponData != null)
            {
                ...
                if (name.Equals("weapon_minigun"))
                {
                    NumBullet = Int32.Parse(arr[0]) - 1;
                }
                else if (name.Equals("weapon_rocket"))
                {
                    NumBullet = Int32.Parse(arr[1]) - 1;
                }
                else if (name.Equals("weapon_napalm"))
                {
                    NumBullet = Int32.Parse(arr[2]) - 1;
                }
                else if (name.Equals("weapon_rocket_A2"))
                {
                    NumBullet = Int32.Parse(arr[3]) - 1;
                }
            }
            file.Close();
